<template>
  <div class="box">
    <section id="jsnode_media_html">
      <h3>音频/视频标签</h3>
      音频标签：&lt;audio&gt;&lt;/audio&gt; <br/>
      视频标签：&lt;video&gt;&lt;/video&gt; <br/><br/>
      若页面无需展示，可以使用js中的：new Audio() 和 new Video() 来获取到对应的操作对象。

    </section>
    <section id="jsnode_media_property">
      <h3>HTML 音频/视频属性</h3>
      <el-table :data="propertyData" border>
        <el-table-column min-width="100" prop="property" label="属性" />
    ...
    <!--  -->
    <section  id="jsnode_media_method">
      <h3>HTML 音频/视频方法</h3>
      <el-table :data="methodData" border>
        <el-table-column min-width="100" prop="property" label="方法名" />
        <el-table-column min-width="300" label="描述">
          <template #default="props">
          <div style="word-break:auto-phrase;" v-html="props.row.desc"></div>
        </template>
    ...
    <!--  -->
    <section  id="jsnode_media_event">
      <h3>HTML 音频/视频事件</h3>
      <el-table :data="eventData" border>
        <el-table-column min-width="100" prop="property" label="事件名" />
...
const propertyData = [
  {
    property: 'autoplay',
    desc: '设置或返回是否在加载完成后随即播放音频/视频。布尔值'
  },
  {
    property: 'preload',
    desc: `设置或返回音频/视频是否在页面加载后经行加载。
    <ol>
  ...
  {
    property: 'buffered',
    desc: `返回TimeRanges对象，TimeRanges对象表示用户的音频/视频缓冲范围，缓冲范围指的是已缓冲
  ...
  {
    property: 'played',
    desc: `返回TimeRanges对象，表示用户已经播放或看到的音频/视频范围。已播放范围指的是被播放
    音频/视频的时间范围。如果用户在音频/视频中跳跃，则会获得多个播放范围。`
  },
  {
    property: 'seekable',
    desc: `返回TimeRanges对象，表示音频/视频中用户可寻址的范围。可寻址范围指的是用户在音频/视频中
    可寻址（移动播放位置）的时间范围。对于非流视频，通常可以寻址到视频中的任何位置，及时其尚未完成缓冲。`
  },
  {
    property: 'controls',
    desc: '设置或返回音频/视频是否显示控件。布尔值'
  },
  {
    property: 'src',
    desc: '设置或返回音频/视频元素的当前来源。'
  },
  {
    property: 'currentSrc',
    desc: '返回当前音频/视频的URL。readonly'
  },
  {
    property: 'currentTime',
    desc: '设置或返回音频/视频中的当前播放位置。'
  },
  {
    property: 'duration',
    desc: '返回当前音频/视频的长度（以秒计）。只读'
  },
  {
    property: 'volume',
    desc: '设置或返回音频/视频的音量，number[0~1]'
  },
  {
    property: 'muted',
    desc: '设置或返回音频/视频是否静音。布尔值'
  },
  {
    property: 'ended',
    desc: '返回当前音频/视频的播放是否已结束。只读'
  },
  {
    property: 'loop',
    desc: '设置或返回音频/视频是否应在结束时重新播放。'
  },
  {
    property: 'paused',
    desc: '返回音频/视频是否暂停，只读，布尔值。'
  },
  {
    property: 'playbackRate',
    desc: '设置或返回音频/视频播放的速度。'
  },
  {
    property: 'mediaGroup',
    desc: '设置或返回音频/视频所属的媒体组合的名称。媒体组合允许两个或更多音频/视频元素保持同步。'
  },
  {
    property: 'networkState',
    desc: `返回音频/视频的当前网络状态。只读，返回一个数字：
    <ol>
  ...
  {
    property: 'seeking',
    desc: '返回用户是否正在音频/视频中进行查找。只读，布尔值。'
  },
  {
    property: '',
    desc: ''
...
const methodData = [
  {
    property: 'canPlayType(type)',
    desc: `检测浏览器是否能播放指定的音频/视频类型。<br/>
    参数type：<br/>
    <ol>
      <li>video/ogg</li>
      <li>video/mp4</li>
